...
CERT_ARN=`aws acm list-certificates | jq --arg host \*.${HOST} '.CertificateSummaryList[] | select(.DomainName==$host) | .CertificateArn'`
aws acm import-certificate --certificate file://${DIR}/cert.pem \
